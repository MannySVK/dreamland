...
public class Gordon extends AbstractNpcAI
{
	...
	public Gordon(String name, String descr)
	{
		...
		startQuestTimer("check_ai", 120000, null, null, 120000);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("check_ai"))
		{
			...
			if (!_isSpawned)
			{
				...
				{
					_isSpawned = true;
					startQuestTimer("Start", 1000, gordon_ai, null, 1000);
		...
		else if (event.equalsIgnoreCase("Start"))
		{
			if (npc != null && _isSpawned)
			{
				...
				if (npc.getNpcId() == GORDON)
				{
					for (L2PcInstance pc : npc.getKnownList().getKnownType(L2PcInstance.class))
					{
						if (pc.isCursedWeaponEquipped() && pc.isInsideRadius(npc, 5000, false, false))
						{
							...
							startQuestTimer("time_isAttacked", 180000, npc, null, 0);
							return super.onAdvEvent(event, npc, player);
						}
					}
				}
	...
	public String onSpawn(L2Npc npc)
	{
		if (npc.getNpcId() == GORDON && _npcBlock == 0)
		{
			_isSpawned = true;
			_isWalkTo = 1;
			startQuestTimer("Start", 1000, npc, null, 1000);
		}
