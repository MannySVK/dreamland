...
public class DrChaos extends AbstractNpcAI
{
	...
	private static final byte DEAD = 0; // Dr. Chaos has been killed and has not yet spawned.
	private static final byte NORMAL = 1; // Dr. Chaos is in NPC form.
	private static final byte CRAZY = 2; // Dr. Chaos entered on golem form.
	...
	public DrChaos(String name, String descr)
	{
		...
		final L2GrandBossData data = GrandBossManager.getInstance().getBossData(CHAOS_GOLEM);
		if (data.getState() == DEAD)
		{
			long temp = data.getRespawn() - System.currentTimeMillis();
			if (temp > 0)
				startQuestTimer("reset_drchaos", temp, null, null, 0);
			else
			{
				...
				GrandBossManager.getInstance().updateBossState(CHAOS_GOLEM, NORMAL);
			}
		}
		// Spawn the war golem.
		else if (data.getState() == CRAZY)
		{
			L2GrandBossInstance golem = (L2GrandBossInstance) addSpawn(CHAOS_GOLEM, data.getLocation(), false, 0, false);
			GrandBossManager.getInstance().addBossInstance(golem);
			golem.setCurrentHpMp(data.getCurrentHp(), data.getCurrentMp());
			golem.setRunning();
			
			// start monitoring Dr. Chaos's inactivity
			_lastAttackVsGolem = System.currentTimeMillis();
			startQuestTimer("golem_despawn", 60000, golem, null, 60000);
		}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("golem_despawn") && npc != null)
		{
			if (npc.getNpcId() == CHAOS_GOLEM)
			{
				if (_lastAttackVsGolem + 1800000 < System.currentTimeMillis())
				{
					...
					GrandBossManager.getInstance().updateBossState(CHAOS_GOLEM, NORMAL); // mark Dr. Chaos is not crazy any more
		...
		else if (event.equalsIgnoreCase("5"))
		{
			...
			// start monitoring Dr. Chaos's inactivity
			_lastAttackVsGolem = System.currentTimeMillis();
			startQuestTimer("golem_despawn", 60000, npc, null, 60000);
		}
		...
		{
			if (GrandBossManager.getInstance().getBossData(CHAOS_GOLEM).getState() == NORMAL)
			{
	...
	public String onSpawn(L2Npc npc)
	{
		...
		startQuestTimer("paranoia_activity", 1000, npc, null, 1000);
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		GrandBossManager.getInstance().updateBossData(CHAOS_GOLEM, false);
		
		cancelQuestTimer("golem_despawn", npc, null);
		startQuestTimer("reset_drchaos", respawnTime, null, null, 0);
		return null;
	}
	...
	private void crazyMidgetBecomesAngry(L2Npc npc)
	{
		if (GrandBossManager.getInstance().getBossData(CHAOS_GOLEM).getState() == NORMAL)
		{
			...
			GrandBossManager.getInstance().updateBossState(CHAOS_GOLEM, CRAZY);
			...
			startQuestTimer("1", 2000, npc, null, 0); // 2 secs, time to launch dr.C anim 2. Cam 1 on.
			startQuestTimer("2", 4000, npc, null, 0); // 2,5 secs, time to launch dr.C anim 3.
			startQuestTimer("3", 6500, npc, null, 0); // 6 secs, time to launch dr.C anim 1.
			startQuestTimer("4", 12500, npc, null, 0); // 4,5 secs to make the NPC moves to the grotto. Cam 2 on.
			startQuestTimer("5", 17000, npc, null, 0); // 4 secs for golem spawn, and golem anim. Cam 3 on.
		}
	}
	
	public static void main(String[] args)
	{
		new DrChaos(DrChaos.class.getSimpleName(), "ai/individual");
	}
}